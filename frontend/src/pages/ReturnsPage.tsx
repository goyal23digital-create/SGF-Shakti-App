...
import { useReturns, useCreateReturn, useVoidReturn, useItems, useParties } from '../api/hooks';
...
export function ReturnsPage() {
  ...
  const voidReturn = useVoidReturn();
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          <h1 className="page-title">Sale Returns</h1>
      ...
      <div className="card">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">New Return Entry</h2>
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
            ...
            <div className="form-field">
              ...
              <div className="input-readonly font-semibold text-green-700">₹{fmt(grandTotal)}</div>
            </div>
          </div>
          <div className="flex items-end gap-4">
            <div className="form-field flex-1">
              <label className="label">Remarks</label>
              <input type="text" className="input" value={form.remarks} onChange={(e) => setForm({ ...form, remarks: e.target.value })} placeholder="Reason for return" />
            </div>
            <button type="submit" className="btn-primary" disabled={create.isPending}>{create.isPending ? 'Saving…' : 'Add Return'}</button>
          </div>
        ...
        {isLoading ? <p className="p-5 text-gray-400">Loading…</p> : (
          <div className="overflow-x-auto">
            <table className="data-table">
              ...
              <tbody>
                {rows.map((r: any) => (
                  <tr key={r.id}>
                    ...
                    <td><span className="badge badge-orange mr-1">{r.item.code}</span>{r.item.name}</td>
              ...
              <tfoot>
                <tr>
                  <td colSpan={7} className="text-right text-xs text-gray-500">Grand Total</td>
                  <td className="text-right font-mono text-green-700">₹{totalGrand.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</td>
                  <td colSpan={2}></td>
                </tr>
              </tfoot>
            </table>
