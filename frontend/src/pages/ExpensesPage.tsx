...
export function ExpensesPage() {
  ...
  const [form, setForm] = useState({ date: today(), payee: '', amount: '', paymentMode: 'CASH', remarks: '' });
  const [error, setError] = useState('');

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    create.mutateAsync({ ...form, fyYear: fy }).then(() => setForm({ ...form, payee: '', amount: '', remarks: '' })).catch((e) => setError(e.response?.data?.error ?? 'Error'));
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Expenses (Cash Out)</h1>
          <p className="text-gray-500 text-sm">Total: ₹{total.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-4 flex flex-wrap gap-3 items-end">
        ...
        <Field label="Payee / Category" value={form.payee} onChange={(v) => setForm({ ...form, payee: v })} required width="w-52" />
        ...
        <button type="submit" className="bg-brand-600 text-white px-4 py-1.5 rounded text-sm hover:bg-brand-700">Add Expense</button>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-auto">
        {isLoading ? <p className="p-4 text-gray-400">Loading…</p> : (
          <table className="table-grid">
            <thead><tr><th>Date</th><th>Payee</th><th className="text-right">Amount</th><th>Mode</th><th>Remarks</th><th></th></tr></thead>
            <tbody>
              {rows.map((r: any) => (
                <tr key={r.id}>
                  ...
                  <td><span className="text-xs bg-gray-100 px-1.5 py-0.5 rounded">{r.paymentMode}</span></td>
                  <td className="text-gray-500 text-xs">{r.remarks}</td>
                  <td><button onClick={() => api.delete(`/expenses/${r.id}`)} className="text-red-500 hover:underline text-xs">Void</button></td>
                </tr>
              ))}
            </tbody>
          </table>
